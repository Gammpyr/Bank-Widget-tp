...
def test_main_events(mock_range, mock_transactions, mock_spending, mock_exchange, mock_stock, mock_df_data):
    ...
    mock_transactions.return_value = pd.DataFrame({
        ...
        "Статус": ["OK", "OK", ],
        "Категория": ["Наличные", "Транспорт", ],
        "Дата платежа": ["01-01-2023", "15-02-2023", ],
    ...
    transfers_and_cash = [{"category": "Наличные", "amount": 1000}, {"category": "Переводы", "amount": 0}, ]
    expected = {
        "expenses": {
            "total_amount": '3000',
            "main": main_test,
        ...
        "income": {
            "total_amount": '3000',
            "main": [{"category": 'Каршеринг', "amount": 3000}]
        },
